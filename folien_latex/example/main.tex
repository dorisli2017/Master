...
\item using the \emph{SLS} with probability ($1-p$) 
\end{itemize}
\begin{algorithm}[H]
\SetKwInOut{Input}{input}
\SetKwInOut{Output}{output}
...
\item if no permitted greefy literal exists, we pick a literal using \emph{SLS} heuristic.
...
\begin{itemize}
\item parallelize flippings in both partitioning sets
\item Two slave thread  deals with partitioning sets in parallel.
\item The master thread handles with conflicts in the intersection.
\item can only solve trivial small problems
\item the order of solving the clauses in search
\item Repeated flippings make the search stuck in one cycling.
\end{itemize}
\begin{figure}[H]
\begin{center}
  \includegraphics[scale = 0.18]{1/a2.png}
  \end{center}
  \label{a2}
  \end{figure}
\end{frame}
...
\begin{itemize}
\item the slave threads take intersection into consideration.
\item The master thread deals with these differences in sub-assignments 
\begin{itemize}
\item \emph{randomCombine}: assigns the variable randomly
\item \emph{partitionCombine}: assigns the variable to the value suggested by its charging thread
\end{itemize}
\item \emph{randomCombine} can not bring a better performance compared with our \emph{swpSAT} solver
\item \emph{partitionCombine} suffers from repeated flips and may not terminate. 
\end{itemize}
\begin{figure}[H]
\begin{center}
  \includegraphics[scale = 0.15]{1/a3.png}
  \end{center}
  \label{a2}
  \end{figure}
\end{frame}
...
\begin{frame}
  ...
  \includegraphics[scale = 0.8]{DATA/BIG/a4.pdf}
\end{frame}
\begin{frame}
  \frametitle{Evaluation}
	\framesubtitle{ benchmark \emph{COMBINE}-\emph{SMALL}}
    \includegraphics[scale = 0.8]{DATA/SMALL/a4.pdf}
\end{frame}
\begin{frame}
...
\item different local variants, which gets better performance than the \emph{probSAT} algorithm. 
...
\item Using different random generation in local search
\end{itemize}
\end{frame}
\begin{frame}
 \begin{figure}
