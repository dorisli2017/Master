...
To use the technique \emph{simulated annealing}, we need some quality to measure the quality of the current solution.  One traditional way is to use the number of conflicts. Besides it, we propose the number of greedy literals in the chosen clause. Our hyperthesis behind this idea is that if the current solution is near a satisfying assignment without conflicts,  it is likely to break a clause by a flipping.  And in such case, there should not be only a few greedy literals exist in the chosen clause. Furthermore,  this local quality is more specific about the chosen clause not generally about the whole search progress.  In the following three experiments, we try to improve our three variants \emph{WALK}, \emph{Average} and \emph{Random-Flip} with simulated annealing and compare these two definitions of qualities. \\
The combination with simulated Annealing show worse performance than the original \emph{WALK} in 3SAT problems. they solve one less problems. The original \emph{Average} can solve $15\%$ more problems in 3SAT. For 5SAT, there are not noticabel differences in aspect of number of solved problems or the runtime. For 7SAT problems, the \emph{Local} version can solve 6  problems more than the original \emph{Average}. It has also shown its efficiency in solving huge 7SAT problems.  The \emph{Local} (with $\alpha = 2$) can solve $10\%$ more problems than the \emph{probSAT} within less than $60\%$ time (in PAR-2 schema).
Besides solving one less problem, the combination with simulated Annealing show also worse efficiency than the original \emph{WALK} for 3SAT insatnces..\\
The performances of these three algorithms are guiet semiliar for small 3SAT problems within $50$ seconds. The \emph{Average} shows more adventages with the increasement of the problems size in 3SAT set. There are not big differences in  \emph{Average} with different combination of simulated annealing for 5SAT and 7SAT set while the variant \emph{Local} has shown  advantages in solving huge problems.
\begin{table}[H]
%\begin{minipage}{\textwidth}                                                                                         
\begin{center}
...
\hline 
    k &\emph{local swpSAT}&\emph{pure portfolio}\\ \hline
	3&11171.3 &\textbf{5626.9} \\ 
	&59 &\textbf{69} \\ \hline
	5&5339.98&\textbf{2185.75}\\ 
	&89 &\textbf{94} \\ \hline
	7&13406.26&\textbf{2853.81}\\
	&66 & \textbf{83} \\ \hline
