...
In section~\ref{sec:Intro}, we summarized the formal concept and techniques used in this paper. Section~\ref{sec:local} described our method combined \emph{probSAT} and \emph{walkSAT}. Then we  discussed some attempts to improve the algorithm. By experimentally evaluation and comparison, some techniques turned out to be more efficient than the simple \emph{probSAT} search. In section~\ref{sec:parallel}, we search the potential benefit of formula partition in a parallel search. Section~\ref{sec:eva} described the details in experiments and several empiric results mentioned in section~\ref{sec:local} and section~\ref{sec:parallel}.  Section~\ref{sec:conc} concluded the paper with further works. 
